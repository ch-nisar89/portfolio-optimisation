...
calculate_performance <- function(weights, returns) {
  ...
  portfolio_returns <- Return.portfolio(R = returns, weights = weights_vector, rebalance_on = "quarters")
...
fitness_function_selection <- function(subset_indices) {
  ...
  if (portfolio_sd == 0 || is.nan(portfolio_sd) || is.infinite(portfolio_sd)) {
    sharpe_ratio <- -Inf  # Assign a large penalty if SD is zero or undefined
  } else {
...
ga_selection <- ga(type = "binary", fitness = fitness_function_selection, 
                   nBits = length(symbols), popSize = 50, maxiter = 100, elitism = 5)
...
ga_result <- ga(type = "real-valued", fitness = fitness_function, 
                ...
                popSize = 50, maxiter = 100, elitism = 5, run = 50)
