...
```{r include=FALSE}
necessary_packages <- c("alphavantager", "GA", "quantmod", "TTR", "xts", "zoo", "PerformanceAnalytics", "dplyr", "reshape2", "tidyr", "ggplot2")
...
```{r}
# Set API key
av_api_key("J4CHYL41MH59475E")
av_api_key()  # This should return your API key
```
...
```{r}
my_assets <- c("OGDC.KA", "HBL.KA", "PSO.KA", "LUCK.KA", "ENGRO.KA", "UBL.KA", "FFC.KA", "POL.KA", "NBP.KA", "TRG.KA")
```
...
```{r}
# Fetching historical price data from Alpha Vantage for each asset
asset_prices <- list()
for(asset in my_assets) {
  getSymbols(asset, src = 'av', from = "2019-01-01", to = "2021-12-31", auto.assign = TRUE)
  asset_prices[[asset]] <- Cl(get(asset))  # Extract closing prices
}

# Merge the prices and remove NAs
combined_prices <- do.call(merge, asset_prices)
combined_prices <- na.omit(combined_prices)

# Display the first few rows of the combined prices
...
ga_result <- ga(type = "real-valued",
                fitness = function(weights) fitness_function_alpha(weights, alpha=0.5, riskFreeRate=0.025),
...
```
- The graph shows how portfolio performance metrics change with different levels of alpha. As alpha increases, the portfolio's risk goes down, but the return and Sharpe Ratio stay roughly the same. 
- There's more uncertainty in risk measurement at the lowest alpha value, as shown by the large error bar.
...
```{r}
av_api_key("J4CHYL41MH59475E")  # Ensure the correct API key is set

# Test with a known symbol
data <- new.env()
getSymbols("MSFT", src = "av", from = "2019-01-01", to = "2021-12-31", env = data, auto.assign = TRUE)

# Check the data
head(get("AAPL", envir = data))
```
...
- In contrast, assets such as 'KO' and 'EQIX' show lower volatility, implying they are less risky, though they may offer more modest returns. 
...
fitness_function_selection <- function(subset_indices) {
  ...
  portfolio_returns <- rowMeans(selected_returns, na.rm = TRUE)  # Ensure NA values are removed
  portfolio_sd <- sd(portfolio_returns, na.rm = TRUE)  # Ensure NA values are removed
  
  # Check for NA, NaN, or infinite values, and set Sharpe ratio to -Inf in these cases
  if (is.na(portfolio_sd) || portfolio_sd == 0 || is.nan(portfolio_sd) || is.infinite(portfolio_sd)) {
    ...
    # Check if Sharpe ratio is undefined or infinite and return -Inf if so
...
## Part 2(d): Visualization of Portfolio Performance (Commulative Return Graph encountered error at last moment)

- Extracting the solution from the GA result and ensuring it's a vector
best_weights <- ga_result@solution
best_weights <- as.vector(best_weights)

- Normalizing the weights so they sum to 1
best_weights <- best_weights / sum(best_weights)

- Ensuring best_weights is a column matrix with the same number of rows as there are columns in log_returns_matrix
weights_matrix <- matrix(best_weights, ncol = 1)

portfolio_log_returns <- log_returns_matrix %*% weights_matrix
...
- Converting trading days to years for the x-axis
time_in_years <- seq_along(cumulative_returns) / 252

- Plotting with years on the x-axis
